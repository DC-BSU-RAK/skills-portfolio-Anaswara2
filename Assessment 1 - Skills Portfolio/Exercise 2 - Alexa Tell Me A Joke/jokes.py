...
import os
from tkinter import messagebox
import pygame  # ADDED: Import pygame for audio
...
LAUGH_PATH = os.path.join(ASSETS, "laugh.mp3")
...
# Load Laugh Sound Effect
laugh_sound = None
if os.path.exists(LAUGH_PATH):
    laugh_sound = pygame.mixer.Sound(LAUGH_PATH)
    laugh_sound.set_volume(0.8) # Optional: Set volume for sound effect
else:
    print(f"Warning: Laugh sound not found at {LAUGH_PATH}")
...
# NEW FUNCTION: Reveal Punchline + Play Sound
def reveal_punchline():
    """
    Shows the punchline text and plays the laugh sound.
    """
    if current_joke:
...
# Create the other buttons using the shared style
# UPDATED: command now calls reveal_punchline instead of lambda
show_btn = tk.Button(root, text="Show",
                     command=reveal_punchline,
                     **button_style)
show_btn.bind("<Enter>", on_enter)
show_btn.bind("<Leave>", on_leave)
